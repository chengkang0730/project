...
class AdminLog extends Base
{
    ...
    public static function record($token)
    {
        ...
        $admin_user = model('common/auth.User')->get($admin_user_id);
        ...
        if(!$title)
        {
            $menus = model('common/auth.Menu')->where('rule','=',$now_node)->order('pid','asc')->select()->toArray();
            if(!$menus){ return true; }
            for($i=1;$i<=4;$i++){
                if($menus[0]['pid']){
                    $new_menu = model('common/auth.Menu')->where('id','=',$menus[0]['pid'])->find();
                    if($new_menu){
