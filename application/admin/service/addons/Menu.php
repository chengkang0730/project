...
class Menu extends Service
{
    ...
    public function __construct(){
        $this->menu_model = new \app\common\model\auth\Menu();
    }
    ...
    public function delete($menu_ids){
        \app\common\model\auth\Menu::where('id','in',$menu_ids)->delete();
        return true;
    }

    //启用菜单
    public function enable($menu_ids){
        \app\common\model\auth\Menu::where('id','in',$menu_ids)->update(['is_hide'=>0]);
        return true;
    }

    //禁用菜单
    public function disable($menu_ids){
        \app\common\model\auth\Menu::where('id','in',$menu_ids)->update(['is_hide'=>1]);
        return true;
    }
}