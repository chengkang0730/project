...
use app\common\model\auth\Menu;
use library\Token;
use library\Tree;
...
class Backend extends Controller
{
    ...
    public function initialize(){
        ...
        if($this->ref){
             ...
             */
            $response = $this->fetch('admin@ltiframe/index');
    ...
    public function auth(){
        ...
        if(!$token){
            if($this->request->isAjax()){
                $this->error('登录信息过期，请重新登录',['iframe_reload'=>true]);
            }else{
        ...
        $this->admin_user = model('common/auth.User')->get($admin_user_id);
        ...
        //当前登录用户角色
        $role_ids = model('common/auth.RoleRelUser')->where('admin_id','=',$admin_user_id)->column('role_id');
        $this->role_ids = $role_ids;
        $menu_ids = array_unique( model('common/auth.RoleRelMenu')->where('role_id','in',$role_ids)->column('menu_id') );
        foreach($menu_ids as $menu_id){
            $pid = model('common/auth.Menu')->where('id','=',$menu_id)->value('pid');
            if($pid){
                for($i=1;$i<=3;$i++){
                    $menu_info = model('common/auth.Menu')->where('id','=',$pid)->find();
        ...
        $rule_list = array_unique( model('common/auth.Menu')->where($where)->column('rule') );
        ...
        if( !in_array($this->now_node, $rule_list) && !in_array($this->action, $this->no_need_login) ){
            ...
            $current_rule_id = model('common/auth.Menu')->where([
                ['rule','=',$current_rule],
                ['is_menu','=',1]
    ...
    public function theme(){
        ...
        $theme = ($cookie !== null) ? Cookie::get('theme') : ((1 <= date('j') && date('j') <= 22) ? date('j') + 1 : 9);
    ...
    public function menu(){
        ...
        $menus = model('common/auth.Menu')->where($menus_where)->order(['pid'=>'asc','sort'=>'desc'])->select()->toArray();
        $this->menus = $menus;
